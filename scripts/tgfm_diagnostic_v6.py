# TGFM diagnostic v6: compact logs + matrix dumps only for failed bootstraps.
# To use: copy/rename this file to tgfm.py so run_tgfm.py imports it.
import sys
import numpy as np 
import os 
import re
...
DIAG_PRINTED_NORMAL_BETA_MU_EXAMPLE = False

# Matrix dump settings. Keep logs compact: print only file paths, save large matrices as .npz.
DIAG_DUMP_DIR_ENV = 'TGFM_DIAG_OUTDIR'
DIAG_DEFAULT_DUMP_DIR = 'tgfm_diagnostic_matrix_dumps'
DIAG_DUMPED_WINDOW_MATRICES = set()
DIAG_DUMPED_FAILED_BS = set()
...
class TGFM(object):
	...
	def _diag_stats(self, x):
		"""Return compact finite-value stats for logging; never prints arrays."""
		x = np.asarray(x)
		finite = x[np.isfinite(x)]
		if finite.size == 0:
			return 'all_nonfinite=True;nan={};inf={}'.format(int(np.sum(np.isnan(x))), int(np.sum(np.isinf(x))))
		abs_finite = np.abs(finite)
		return 'min={:.3e};q50={:.3e};q99_abs={:.3e};max_abs={:.3e};nan={};inf={};zero={}'.format(
			np.min(finite),
			np.median(finite),
			np.quantile(abs_finite, 0.99),
			np.max(abs_finite),
			int(np.sum(np.isnan(x))),
			int(np.sum(np.isinf(x))),
			int(np.sum(x == 0.0))
		)

	def _diag_bad(self, x, extreme=1e50, small=None):
		"""Flag NaN/Inf/extreme values. If small is set, also flag tiny non-zero absolute values."""
		x = np.asarray(x)
		if np.any(~np.isfinite(x)):
			return True
		if np.any(np.abs(x) > extreme):
			return True
		if small is not None:
			finite = x[np.isfinite(x)]
			if np.any((np.abs(finite) < small) & (finite != 0.0)):
				return True
		return False

	def _diag_update_is_bad(self, bs_iter, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term):
		"""Decide whether this bootstrap update is numerically problematic."""
		bad_beta = (
			self._diag_bad(variant_b_terms, extreme=1e50) or
			self._diag_bad(variant_a_terms, extreme=1e50, small=1e-12) or
			self._diag_bad(mixture_beta_var, extreme=1e50) or
			self._diag_bad(mixture_beta_mu, extreme=1e50) or
			self._diag_bad(mixture_beta_phi, extreme=1e20) or
			self._diag_bad(un_normalized_lv_beta_weights, extreme=700)
		)
		bad_alpha = (
			self._diag_bad(b_terms, extreme=1e50) or
			self._diag_bad(a_terms, extreme=1e50, small=1e-12) or
			self._diag_bad(mixture_alpha_var, extreme=1e50) or
			self._diag_bad(mixture_alpha_mu, extreme=1e50) or
			self._diag_bad(mixture_alpha_phi, extreme=1e20) or
			self._diag_bad(un_normalized_lv_alpha_weights, extreme=700)
		)
		bad_residual = self._diag_bad(self.residual[bs_iter, :], extreme=1e50) or self._diag_bad(self.residual_incl_alpha[bs_iter, :], extreme=1e50)
		bad_norm = not np.isfinite(normalizing_term)
		return bad_beta, bad_alpha, bad_residual, bad_norm

	def _diag_print_normal_beta_mu_once(self, itera, l_index, bs_iter, mixture_beta_mu, mixture_beta_phi, variant_b_terms, variant_a_terms, mixture_beta_var, un_normalized_lv_beta_weights, normalizing_term):
		"""Print exactly one finite, non-extreme beta_mu example across the Python process."""
		global DIAG_PRINTED_NORMAL_BETA_MU_EXAMPLE
		if DIAG_PRINTED_NORMAL_BETA_MU_EXAMPLE:
			return
		if self._diag_bad(mixture_beta_mu, extreme=1e20):
			return
		if self._diag_bad(mixture_beta_phi, extreme=1e20):
			return
		finite_vbt = variant_b_terms[np.isfinite(variant_b_terms)]
		finite_vat = variant_a_terms[np.isfinite(variant_a_terms)]
		finite_mbv = mixture_beta_var[np.isfinite(mixture_beta_var)]
		finite_lw = un_normalized_lv_beta_weights[np.isfinite(un_normalized_lv_beta_weights)]
		print('TGFM_NORMAL_BETA_MU_EXAMPLE',
			'trait=' + str(getattr(self, 'trait_name', 'NA')),
			'window_name=' + str(getattr(self, 'window_name', 'NA')),
			'iteration=' + str(itera),
			'component=' + str(l_index),
			'bs_iter=' + str(bs_iter),
			'beta_mu_stats=' + self._diag_stats(mixture_beta_mu),
			'beta_phi_sum={:.6e}'.format(np.sum(mixture_beta_phi)),
			'beta_phi_max={:.6e}'.format(np.max(mixture_beta_phi)),
			'variant_b_terms_max_abs={:.6e}'.format(np.max(np.abs(finite_vbt)) if finite_vbt.size > 0 else np.nan),
			'variant_a_terms_min_abs={:.6e}'.format(np.min(np.abs(finite_vat)) if finite_vat.size > 0 else np.nan),
			'mixture_beta_var_max_abs={:.6e}'.format(np.max(np.abs(finite_mbv)) if finite_mbv.size > 0 else np.nan),
			'beta_log_weight_max={:.6e}'.format(np.max(finite_lw) if finite_lw.size > 0 else np.nan),
			'normalizing_term={:.6e}'.format(normalizing_term))
		DIAG_PRINTED_NORMAL_BETA_MU_EXAMPLE = True


	def _diag_safe_name(self, x):
		"""Make a safe short string for filenames."""
		x = str(x)
		x = re.sub(r'[^A-Za-z0-9_.-]+', '_', x)
		return x[:120]

	def _diag_outdir(self):
		"""Directory for .npz matrix dumps.
		Set TGFM_DIAG_OUTDIR=/path/to/log_dir if you want dumps next to the log file.
		"""
		outdir = os.environ.get(DIAG_DUMP_DIR_ENV, DIAG_DEFAULT_DUMP_DIR)
		os.makedirs(outdir, exist_ok=True)
		return outdir

	def _diag_dump_window_matrices_once(self, itera, l_index, bs_iter):
		"""Save window-level matrices once per TGFM object/window, not once per failed BS."""
		global DIAG_DUMPED_WINDOW_MATRICES
		trait = getattr(self, 'trait_name', 'NA')
		window = getattr(self, 'window_name', 'NA')
		key = (trait, window, id(self))
		if key in DIAG_DUMPED_WINDOW_MATRICES:
			return 'already_dumped'

		outdir = self._diag_outdir()
		fname = 'tgfm_window_matrices_trait_{}_window_{}_iter_{}_component_{}_first_failed_bs_{}.npz'.format(
			self._diag_safe_name(trait),
			self._diag_safe_name(window),
			itera,
			l_index,
			bs_iter
		)
		...
		np.savez_compressed(
			path,
			trait_name=np.asarray(str(trait)),
			window_name=np.asarray(str(window)),
			iteration=np.asarray(itera),
			component=np.asarray(l_index),
			first_failed_bs=np.asarray(bs_iter),
			reference_ld=self.reference_ld,
			D_mat=self.D_mat,
			srs_inv=self.srs_inv,
			s_inv_2_diag=self.s_inv_2_diag,
			D_diag=self.D_diag,
			gwas_beta=self.gwas_beta,
			gwas_beta_se=self.gwas_beta_se,
			gwas_variant_z=self.gwas_variant_z,
			gene_tissue_pairs=np.asarray(self.gene_tissue_pairs),
			gene_names=np.asarray(self.gene_names),
			variants=np.asarray(self.variants)
		)
		DIAG_DUMPED_WINDOW_MATRICES.add(key)
		print('TGFM_WINDOW_MATRIX_DUMP',
			'trait=' + str(trait),
			'window_name=' + str(window),
			'iteration=' + str(itera),
			'component=' + str(l_index),
			'first_failed_bs=' + str(bs_iter),
			'file=' + str(path))
		return path

	def _diag_dump_failed_bs_matrices(self, itera, l_index, bs_iter, gene_eqtl_pmces, bs_eqtls_pmces_sparse, gene_weights, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term):
		"""Save the dense PMCES and BS-specific matrices/terms for a failed bootstrap only."""
		global DIAG_DUMPED_FAILED_BS
		trait = getattr(self, 'trait_name', 'NA')
		window = getattr(self, 'window_name', 'NA')
		key = (trait, window, id(self), itera, l_index, bs_iter)
		if key in DIAG_DUMPED_FAILED_BS:
			return 'already_dumped'

		# Save the window-wide matrices once; save BS-specific objects once per failed BS.
		self._diag_dump_window_matrices_once(itera, l_index, bs_iter)

		outdir = self._diag_outdir()
		fname = 'tgfm_failed_bs_matrices_trait_{}_window_{}_iter_{}_component_{}_bs_{}.npz'.format(
			self._diag_safe_name(trait),
			self._diag_safe_name(window),
			itera,
			l_index,
		...
		np.savez_compressed(
			...
			iteration=np.asarray(itera),
			component=np.asarray(l_index),
			bs_iter=np.asarray(bs_iter),

			# The key requested object: dense PMCES for this failed bootstrap.
			gene_eqtl_pmces_dense=gene_eqtl_pmces,
			gene_eqtl_pmces_sparse=bs_eqtls_pmces_sparse,

			# Bootstrap-specific precomputed/input-derived terms.
			precomputed_a_terms_bs=self.precomputed_a_terms[bs_iter],
			precomputed_gene_gene_terms_bs=self.precomputed_gene_gene_terms[bs_iter],
			nominal_twas_z_bs=self.nominal_twas_z[bs_iter],
			residual_bs=self.residual[bs_iter, :],
			residual_incl_alpha_bs=self.residual_incl_alpha[bs_iter, :],

			# Current update quantities, saved for matching the compact log.
			gene_weights=gene_weights,
			alpha_b_terms=b_terms,
			alpha_a_terms=a_terms,
			mixture_alpha_var=mixture_alpha_var,
			mixture_alpha_mu=mixture_alpha_mu,
			mixture_alpha_phi=mixture_alpha_phi,
			variant_b_terms=variant_b_terms,
			variant_a_terms=variant_a_terms,
			mixture_beta_var=mixture_beta_var,
			mixture_beta_mu=mixture_beta_mu,
			mixture_beta_phi=mixture_beta_phi,
			un_normalized_lv_alpha_weights=un_normalized_lv_alpha_weights,
			un_normalized_lv_beta_weights=un_normalized_lv_beta_weights,
			normalizing_term=np.asarray(normalizing_term),
			component_variance=np.asarray(self.component_variances[bs_iter][l_index]),

			# Names are useful for tracing rows/columns.
			gene_tissue_pairs=np.asarray(self.gene_tissue_pairs),
			gene_names=np.asarray(self.gene_names),
			variants=np.asarray(self.variants)
		)
		DIAG_DUMPED_FAILED_BS.add(key)
		print('TGFM_FAILED_BS_MATRIX_DUMP',
			'trait=' + str(trait),
			'window_name=' + str(window),
			'iteration=' + str(itera),
			'component=' + str(l_index),
	...
	def _diag_print_failed_bs(self, itera, l_index, bs_iter, bad_beta, bad_alpha, bad_residual, bad_norm, gene_eqtl_pmces, bs_eqtls_pmces_sparse, gene_weights, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term):
		"""Print compact failed-BS stats, and save dense PMCES/window matrices to .npz."""
		self._diag_dump_failed_bs_matrices(itera, l_index, bs_iter, gene_eqtl_pmces, bs_eqtls_pmces_sparse, gene_weights, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term)
		finite_beta_mu = mixture_beta_mu[np.isfinite(mixture_beta_mu)]
		max_abs_beta_mu = np.max(np.abs(finite_beta_mu)) if finite_beta_mu.size > 0 else np.nan
		extreme_beta_indices = np.where(np.abs(mixture_beta_mu) > 1e50)[0]
		nan_beta_phi_indices = np.where(np.isnan(mixture_beta_phi))[0]
		print('TGFM_FAILED_BS',
			'trait=' + str(getattr(self, 'trait_name', 'NA')),
			'window_name=' + str(getattr(self, 'window_name', 'NA')),
			'iteration=' + str(itera),
			'component=' + str(l_index),
			'bs_iter=' + str(bs_iter),
			'bad_beta=' + str(bad_beta),
			'bad_alpha=' + str(bad_alpha),
			'bad_residual=' + str(bad_residual),
			'bad_normalizing_term=' + str(bad_norm),
			'max_abs_beta_mu=' + str(max_abs_beta_mu),
			'n_beta_mu_abs_gt_1e50=' + str(len(extreme_beta_indices)),
			'n_beta_phi_nan=' + str(len(nan_beta_phi_indices)),
			'top_extreme_beta_indices=' + str(extreme_beta_indices[:5]),
			'top_nan_beta_phi_indices=' + str(nan_beta_phi_indices[:5]))
		print('TGFM_FAILED_BS_BETA_CHAIN',
			'bs_iter=' + str(bs_iter),
			'variant_b_terms=' + self._diag_stats(variant_b_terms),
			'variant_a_terms=' + self._diag_stats(variant_a_terms),
			'mixture_beta_var=' + self._diag_stats(mixture_beta_var),
			'mixture_beta_mu=' + self._diag_stats(mixture_beta_mu),
			'beta_log_weights=' + self._diag_stats(un_normalized_lv_beta_weights),
			'beta_phi=' + self._diag_stats(mixture_beta_phi),
			'normalizing_term=' + str(normalizing_term),
			'component_variance=' + str(self.component_variances[bs_iter][l_index]))
		print('TGFM_FAILED_BS_ALPHA_INPUT_CHAIN',
			'bs_iter=' + str(bs_iter),
			'alpha_b_terms=' + self._diag_stats(b_terms),
			'alpha_a_terms=' + self._diag_stats(a_terms),
			'mixture_alpha_var=' + self._diag_stats(mixture_alpha_var),
			'mixture_alpha_mu=' + self._diag_stats(mixture_alpha_mu),
			'alpha_log_weights=' + self._diag_stats(un_normalized_lv_alpha_weights),
			'alpha_phi=' + self._diag_stats(mixture_alpha_phi),
			'residual=' + self._diag_stats(self.residual[bs_iter, :]),
			'residual_incl_alpha=' + self._diag_stats(self.residual_incl_alpha[bs_iter, :]))
	...
	def __init__(self, L=10, max_iter=5, gene_log_pi=None, variant_log_pi=None):
		...
		self.diag_stop_logging_for_window = False
		self.diag_failed_component_key = None
	def fit(self, tgfm_data_obj, phi_init=None, mu_init=None, mu_var_init=None):
		...
		for itera in range(self.max_iter):
			...
			self.update_susie_effects(self.gene_log_pi, self.variant_log_pi, self.component_variances, self.component_variances, itera)
	...
	def update_susie_effects(self, expected_log_pi, expected_log_variant_pi, alpha_component_variances, beta_component_variances, itera):
		...
		for l_index in range(self.L):
			...
			for bs_iter in range(self.n_bs):
				...
				# Compact diagnostics: one normal beta_mu example, then all failed bootstraps in the first failed component only
				self._diag_print_normal_beta_mu_once(itera, l_index, bs_iter, mixture_beta_mu, mixture_beta_phi, variant_b_terms, variant_a_terms, mixture_beta_var, un_normalized_lv_beta_weights, normalizing_term)
				if not getattr(self, 'diag_stop_logging_for_window', False):
					bad_beta, bad_alpha, bad_residual, bad_norm = self._diag_update_is_bad(bs_iter, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term)
					if bad_beta or bad_alpha or bad_residual or bad_norm:
						current_key = (itera, l_index)
						if self.diag_failed_component_key is None:
							self.diag_failed_component_key = current_key
							print('TGFM_FAILED_COMPONENT_START',
								'trait=' + str(getattr(self, 'trait_name', 'NA')),
								'window_name=' + str(getattr(self, 'window_name', 'NA')),
								'iteration=' + str(itera),
								'component=' + str(l_index),
								'mode=print_all_failed_bs_in_this_component_then_suppress_later_components')
						if self.diag_failed_component_key == current_key:
							self._diag_print_failed_bs(itera, l_index, bs_iter, bad_beta, bad_alpha, bad_residual, bad_norm, gene_eqtl_pmces, bs_eqtls_pmces_sparse, gene_weights, b_terms, a_terms, mixture_alpha_var, mixture_alpha_mu, mixture_alpha_phi, variant_b_terms, variant_a_terms, mixture_beta_var, mixture_beta_mu, mixture_beta_phi, un_normalized_lv_alpha_weights, un_normalized_lv_beta_weights, normalizing_term)
			...
			# If this component is the first failed component, do not continue diagnostic logging to later components in this window.
			# The model code itself continues only through the current component's bootstraps before suppression; this file is for diagnostics, not final PIP production.
			if self.diag_failed_component_key == (itera, l_index):
				print('TGFM_FAILED_COMPONENT_END',
					'trait=' + str(getattr(self, 'trait_name', 'NA')),
					'window_name=' + str(getattr(self, 'window_name', 'NA')),
					'iteration=' + str(itera),
					'component=' + str(l_index),
					'action=suppress_later_component_diagnostics_for_this_window')
				self.diag_stop_logging_for_window = True
				break
		return
	...
	def update_component_variances(self):
		...
		for bs_iter in range(self.n_bs):
			for l_iter in range(self.L):
				...
				if (not getattr(self, 'diag_stop_logging_for_window', False)) and (not np.isfinite(self.component_variances[bs_iter][l_iter]) or self.component_variances[bs_iter][l_iter] <= 0.0 or self.component_variances[bs_iter][l_iter] > 1e50):
					print('TGFM_COMPONENT_VARIANCE_FIRST_FAILURE', 'trait=' + str(getattr(self, 'trait_name', 'NA')), 'window_name=' + str(getattr(self, 'window_name', 'NA')), 'bs_iter=' + str(bs_iter), 'component=' + str(l_iter), 'component_variance=' + str(self.component_variances[bs_iter][l_iter]))
					self.diag_stop_logging_for_window = True
	...
	def initialize_variables(self, tgfm_data_obj, phi_init, mu_init, mu_var_init):
		...
		self.variants = tgfm_data_obj['variants']
		# GWAS sample size
		self.NN = tgfm_data_obj['gwas_sample_size']

		# Optional metadata used only for diagnostics
		self.window_name = tgfm_data_obj.get('window_name', 'NA')
		self.trait_name = tgfm_data_obj.get('trait_name', 'NA')

		# Get gwas z-scores with minimal repair.
		# Important: do NOT set SE to 1e6. If beta is bad but SE is valid, set beta=0 and keep SE.
		# If SE is bad/tiny, set beta=0 and replace SE with median valid SE within this window.
		gwas_beta = np.copy(tgfm_data_obj['gwas_beta'])
		gwas_beta_se = np.copy(tgfm_data_obj['gwas_beta_se'])
		bad_beta = ~np.isfinite(gwas_beta)
		bad_se = ~np.isfinite(gwas_beta_se)
		tiny_se = np.isfinite(gwas_beta_se) & (np.abs(gwas_beta_se) < 1e-12)
		bad_or_tiny_se = bad_se | tiny_se
		self.n_bad_beta_original = int(np.sum(bad_beta))
		self.n_bad_se_original = int(np.sum(bad_se))
		self.n_tiny_se_original = int(np.sum(tiny_se))
		self.n_repaired_se = int(np.sum(bad_or_tiny_se))
		if np.any(bad_beta) or np.any(bad_or_tiny_se):
			valid_se = gwas_beta_se[np.isfinite(gwas_beta_se) & (np.abs(gwas_beta_se) >= 1e-12)]
			replacement_se = np.median(valid_se) if valid_se.size > 0 else 1.0
			print('TGFM_GWAS_REPAIR_SUMMARY',
				'trait=' + str(self.trait_name),
				'window_name=' + str(self.window_name),
				'n_bad_beta=' + str(self.n_bad_beta_original),
				'n_bad_se=' + str(self.n_bad_se_original),
				'n_tiny_se=' + str(self.n_tiny_se_original),
				'n_repaired_se=' + str(self.n_repaired_se),
				'replacement_se=' + str(replacement_se))
			gwas_beta[bad_beta | bad_or_tiny_se] = 0.0
			gwas_beta_se[bad_or_tiny_se] = replacement_se
		tgfm_data_obj['gwas_beta'] = gwas_beta
		tgfm_data_obj['gwas_beta_se'] = gwas_beta_se
		self.gwas_beta = gwas_beta
		self.gwas_beta_se = gwas_beta_se
		self.gwas_variant_z = gwas_beta/gwas_beta_se
		...
		self.reference_ld = tgfm_data_obj['reference_ld']
		self.D_mat = D_mat
		self.srs_inv = srs_inv_mat
		self.s_inv_2_diag = np.diag(S_inv_2_mat)
		self.D_diag = np.diag(D_mat)
		if self._diag_bad(self.s_inv_2_diag, extreme=1e50) or self._diag_bad(self.D_diag, extreme=1e50, small=1e-12) or self._diag_bad(self.srs_inv, extreme=1e50):
			print('TGFM_INPUT_SCALING_WARNING', 'trait=' + str(self.trait_name), 'window_name=' + str(self.window_name), 's_inv_2_diag=' + self._diag_stats(self.s_inv_2_diag), 'D_diag=' + self._diag_stats(self.D_diag), 'srs_inv=' + self._diag_stats(self.srs_inv))
