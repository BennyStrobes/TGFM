...
def tgfm_inference_shell(tgfm_data, gene_log_prior, var_log_prior, n_components, gene_tissue_pip_threshold):
	...
	print("Variant Z-scores:", np.isnan(variant_z).sum(), "nans")
	print("Variant Z-score shape:", variant_z.shape)

	# Run tgfm (with null/default initialization)
	tgfm_obj = tgfm.TGFM(L=n_components, gene_log_pi=gene_log_prior, variant_log_pi=var_log_prior, max_iter=5) #initialize the TGFM object
	tgfm_obj.fit(tgfm_data_obj=tgfm_data) # run fine-mapping
...
def extract_tissue_specific_log_prior_probabilities_window_genetic_elements(tissue_specific_prior, variant_names, tissue_names):
	...
	normalizers = np.sum(gene_probs,axis=0) + np.sum(var_probs,axis=0) # this ensure the log variant prior + log gene-tissue prior = 1 per bootstrap sample
...
for window_iter in range(n_windows):
	...
	try:
		...
		print(f'WARNING: LD file missing for window {window_name}: {ld_file}')
		t_pip.write(window_name + '\tNA\tNA\n')
		continue
