...
class TGFM(object):
	...
	def compute_gene_pips(self):
		...
		if np.isnan(expected_gene_pips).any():
			nan_indices = np.where(np.isnan(expected_gene_pips))[0]
			print(f"NaNs found in gene PIPs at indices: {nan_indices}")
	...
	def compute_pips(self):
		...
		self.expected_beta_pips = np.mean(self.beta_pips,axis=0)
		    ...
		    # Corrected NaN checks
		if np.isnan(self.expected_alpha_pips).any():
			nan_indices = np.where(np.isnan(self.expected_alpha_pips))[0]
			print(f"NaNs found in gene-tissue pair PIPs at indices: {nan_indices}")

		if np.isnan(self.expected_beta_pips).any():
			nan_indices = np.where(np.isnan(self.expected_beta_pips))[0]
			print(f"NaNs found in variant PIPs at indices: {nan_indices}")
	...
	def update_susie_effects(self, expected_log_pi, expected_log_variant_pi, alpha_component_variances, beta_component_variances):
		...
		for l_index in range(self.L):
			...
			bad_mask = np.isnan(self.beta_mus[l_index]) | np.isnan(self.beta_phis[l_index])
			if np.any(bad_mask):
				rows, cols = np.where(bad_mask)
				for r, c in zip(rows, cols):
					print(f"NaN in beta_*: layer {l_index}, bs_iter={r}, k={c}, beta_mu={self.beta_mus[l_index][r,c]}, beta_phi={self.beta_phis[l_index][r,c]}")
			...
			if np.isnan(component_non_med_pred).any():
				print(f"NaNs in component_non_med_pred at component {l_index}")
			if np.isnan(self.srs_inv).any():
				print("NaNs in srs_inv matrix")
			self.residual = self.residual + np.dot(self.gene_trait_pred[l_index] - self.gene_trait_pred[prev_l_index] + component_non_med_pred, self.srs_inv)  # Residual with both variant-trait effects and variant-gene-trait effects removed -> that's the non-mediated variants
			print("\nResidual before update:")
			print(self.residual[l_index,:])
			...
			for bs_iter in range(self.n_bs):
				...
				if np.isnan(self.residual_incl_alpha[bs_iter, :]).any():
					print(f"NaNs in residual_incl_alpha[{bs_iter}, :]") # nan is from residual_incl_alpha !!!
				if np.isnan(self.s_inv_2_diag).any():
					print("NaNs in s_inv_2_diag")
				if np.isnan(gene_eqtl_pmces).any():
					print(f"NaNs in gene_eqtl_pmces for bootstrap {bs_iter}")
				if np.isnan(self.precomputed_gene_gene_terms[bs_iter]).any():
					print(f"NaNs in precomputed_gene_gene_terms[{bs_iter}]")
				...
				if np.isnan(gene_weights).any():
					print(f"NaNs in gene_weights for bs_iter={bs_iter}")
				...
				# Print mixture_alpha_mu for this component and bootstrap
				#print(f"\nComponent {l_index}, Bootstrap {bs_iter}:")
				#print("mixture_alpha_mu (gene effects):")
				#print(mixture_alpha_mu)

				# safety check for the inner loop per bootstrap
				if np.any(np.isnan(mixture_alpha_var)):
					print(f"NaN in mixture_alpha_var: a_terms={a_terms}")
				if np.any(np.isnan(mixture_alpha_mu)):
					print(f"NaN in mixture_alpha_mu: b_terms={b_terms}")
				...
				# Debug NaN in variant_b_terms
				if np.any(np.isnan(variant_b_terms)):
					# Check which component has NaNs
					residual_nan = np.isnan(self.residual[bs_iter, :])
					s_inv_nan = np.isnan(self.s_inv_2_diag)
					if np.any(residual_nan):
						print(f"NaNs in self.residual at bs_iter={bs_iter}")
					if np.any(s_inv_nan):
						print(f"NaNs in self.s_inv_2_diag")
				...
				###############
				# expected_log_pi is the PIP for genes
				...
				# expected_log_variant_pi is the PIP for variants
				...
				# safety check for the normalzied alpha/beta terms
				if np.any(np.isnan(un_normalized_lv_alpha_weights)):
					print("NaNs in un_normalized_lv_alpha_weights")
				if np.isnan(normalizing_term):
					print("NaN in normalizing_term")
				...
				# Save results (PIP) to global model parameters
	...
	def initialize_variables(self, tgfm_data_obj, phi_init, mu_init, mu_var_init):
		...
		########################################
		# Remove variants with NaN GWAS beta
		########################################
		valid_variants = ~np.isnan(tgfm_data_obj['gwas_beta'])

		if np.sum(~valid_variants) > 0:
			print('Removing', np.sum(~valid_variants), 'variants with NaN gwas_beta')

    		# Variant-level vectors
			tgfm_data_obj['gwas_beta'] = tgfm_data_obj['gwas_beta'][valid_variants]
			tgfm_data_obj['gwas_beta_se'] = tgfm_data_obj['gwas_beta_se'][valid_variants]
			tgfm_data_obj['variants'] = tgfm_data_obj['variants'][valid_variants]

    		# LD matrix
			tgfm_data_obj['reference_ld'] = tgfm_data_obj['reference_ld'][valid_variants, :]
			tgfm_data_obj['reference_ld'] = tgfm_data_obj['reference_ld'][:, valid_variants]

    		# Gene eQTL PMCES
			new_pmces = []
			for bs_iter in range(len(tgfm_data_obj['sparse_sampled_gene_eqtl_pmces'])):
				tmp = tgfm_data_obj['sparse_sampled_gene_eqtl_pmces'][bs_iter].copy()

        		# old variant index stored in column 1
				keep = valid_variants[tmp[:,1].astype(int)]
				tmp = tmp[keep,:]

        		# remap indices
				old_to_new = -np.ones(len(valid_variants), dtype=int)
				old_to_new[valid_variants] = np.arange(np.sum(valid_variants))
				tmp[:,1] = old_to_new[tmp[:,1].astype(int)]
				
				new_pmces.append(tmp)
			
			tgfm_data_obj['sparse_sampled_gene_eqtl_pmces'] = new_pmces
		########################################
		...
		if np.any(s_vec == 0.0):
			print("Zero detected in s_vec! Leads to inf in S_inv_mat.")
		S_mat = np.diag(s_vec)
		...
		# Check the nan in initialized input
		assert not np.isnan(self.gwas_variant_z).any(), "NaNs in gwas z scores"
		assert not np.isnan(tgfm_data_obj['reference_ld']).any(), "NaNs in reference_ld"
		assert not np.isnan(self.s_inv_2_diag).any(), "NaNs in s_inv_2_diag"
		...
		if phi_init is not None and mu_init is not None and mu_var_init is not None:
			assert not np.isnan(phi_init).any(), "phi_init has NaNs"
			assert not np.isnan(mu_init).any(), "mu_init has NaNs"
			assert not np.isnan(mu_var_init).any(), "mu_var_init has NaNs"
