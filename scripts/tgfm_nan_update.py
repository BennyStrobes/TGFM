...
class TGFM(object):
	...
	def initialize_variables(self, tgfm_data_obj, phi_init, mu_init, mu_var_init):
		...
		# Handle NaN in gwas_beta by setting to 0 and increasing SE
		nan_mask = np.isnan(tgfm_data_obj['gwas_beta'])
		if np.any(nan_mask):
			print(f"Warning: {np.sum(nan_mask)} variants have NaN gwas_beta. Setting to 0 with large SE.")
			tgfm_data_obj['gwas_beta'][nan_mask] = 0.0
			tgfm_data_obj['gwas_beta_se'][nan_mask] = 1e6  # Large SE to minimize impact

		# Now compute z-scores with safeguards
		with np.errstate(divide='ignore', invalid='ignore'):
			gwas_z = tgfm_data_obj['gwas_beta'] / tgfm_data_obj['gwas_beta_se']
			invalid_z = np.isnan(gwas_z)
			if np.any(invalid_z):
				print(f"Correcting {np.sum(invalid_z)} invalid z-scores (nan)")
				gwas_z[invalid_z] = 0.0  # Set invalid z-scores to 0
		
		# Proceed with initialization using sanitized data
		self.gwas_variant_z = gwas_z
		# GWAS sample size
